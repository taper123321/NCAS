...
public:
    ...
    static const size_type npos = -1;		///< Value that means the end of string.
public:
...
inline string operator+ (const char* cs, const string& ss) { string r; r.reserve (strlen(cs)+ss.size()); r += cs; r += ss; return (r); }
 ...
 inline string operator+ (const string& ss,const char *cs) { string r; r.reserve (strlen(cs)+ss.size()); r += ss; r += cs; return (r); }

inline string operator+ (char c, const string& ss) { string r; r.reserve (1+ss.size()); r += c; r += ss; return (r); }

 inline string operator+ (const string& ss,char c) { string r; r.reserve (1+ss.size()); r += ss; r += c; return (r); }
