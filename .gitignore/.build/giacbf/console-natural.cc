...
char session_filename[MAX_FILENAME_SIZE+1]="natcas";
    ...
    if (key==KEY_CTRL_F6){
	  ...
	  if (smallmenu.selection==4){
	    ...
	    if (fileBrowser(filename, (char*)"*.xw", (char *)"Sessions")){
	      if (console_changed==0 || strcmp(session_filename,"natcas")==0 || confirm(lang?"Session courante perdue?":"Current session will be lost",lang?"F1: annul, F6: ok":"F1: cancel, F6: ok")==KEY_CTRL_F6){
