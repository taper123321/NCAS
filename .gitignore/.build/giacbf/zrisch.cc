  ...
  */
  ...
  gen risch(const gen & e_orig,const identificateur & x,gen & remains_to_integrate,GIAC_CONTEXT){
    ...
    for (;it!=itend;){
      ...
      gen expo=*it;
      ++it; rem=0;
      //confirm("risch",expo.print().c_str());
      gen resadd=risch_lin(coeff*exp(expo,contextptr),x,rem,contextptr);
      //confirm("resadd",resadd.print().c_str());
      res = res+resadd;
      remsum += rem;
    }
    // confirm("res",res.print().c_str());
