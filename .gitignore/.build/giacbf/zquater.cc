...
namespace giac {
  ...
  gen _galois_field(const gen & args,GIAC_CONTEXT){
    return gensizeerr("Not implemented. Requires RTTI support (no NO_RTTI in config.h/no -fno-rtti flag in Makefile)");
  }
  ...
  vector<int> operator %(const vector<int> & v,int p){
