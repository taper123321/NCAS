...
extern "C" int printmsg12(const char *,const char *);
...
inline int mpfr_add(mpfr_t & r,const mpfr_t & a,const mpfr_t &b,bf_flags_t flag){
  // char buf[128]; sprintf(buf,"%i",bf_global_prec); printmsg12("mpfr_add",buf);
...
inline char * mpfr_get_str(char * s,mp_exp_t * expo,int base,int prec,const mpz_t &  z,bf_rnd_t round){
  ...
  size_t len;
  char * ptr=bf_ftoa(&len, &z, base, prec,
  ...
  *expo=z.expn;
  //char buf[128]; sprintf(buf,"get_str %i %i",bf_global_prec,*expo); printmsg12(buf,ptr);
  strcpy(s,ptr);
  free(ptr);
  return s;
}
inline int mpfr_set_str (mpfr_t & rop, const char *s, int base, mpfr_rnd_t rnd){
  slimb_t exponent;
  const char * next=0;
  int res=bf_atof2(&rop,&exponent,s,&next,base,bf_global_prec,rnd);
  //mp_exp_t expo; char * ptr=bf_ftoa(&expo, &rop, base, bf_global_prec,rnd); char buf[128]; sprintf(buf,"set_str %i",bf_global_prec); printmsg12(buf,ptr); free(ptr);
  //char buf[128]; sprintf(buf,"set_str %i %c %p",bf_global_prec,next); printmsg12(buf,s);  
  return res;
}
