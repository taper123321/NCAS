...
void MENU_save(){
  if (console_changed){
    save("natcas");
    if (strcmp(session_filename,"natcas")==0)
      console_changed=0;
  }
}  
...
bool naturalcas_save();
void quit_handler(){
  if (cpu_speed>0)
    clock_set_speed(cpu_speed);
  if(!naturalcas_save())save_session();
}
...
int main1(){
  ...
  if (0 &&
    ...
    if (key==KEY_CTRL_DEL && do_confirm(lang==1?"Effacer natcas.xw?":"Erase natcas.xw?")){
      unsigned short pFile[MAX_FILENAME_SIZE+1];
      const char filename[]="\\\\fls0\\natcas.xw";
      Bfile_StrToName_ncpy(pFile, (const unsigned char *) filename, strlen(filename)+1);
  ...
  if (//1 ||
    ...
    if (*(unsigned *)addr!=exammodeptr || doit){
      confirm("1ere utilisation de KhiCAS","natcas.xw va etre efface");
...
void save(const char * fname){
  ...
  save_console_state_smem(("\\\\fls0\\"+filename+".xw").c_str(),strcmp(filename,"natcas")); // call before save_khicas_symbols_smem(), because this calls create_data_folder if necessary!
...
void save_session(){
  if (strcmp(session_filename,"natcas") && console_changed){
  ...
  if (edptr && edptr->changed && edptr->filename!="\\\\fls0\\natcas.py"){
